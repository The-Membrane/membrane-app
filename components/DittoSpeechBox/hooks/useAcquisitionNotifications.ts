...
import { useCurrentAcquisition, useUserAcquisitionDeposits, useAcquisitionConfig, useAcquisition } from '@/hooks/useAcquisition'
...
export const useLockdropEnding = () => {
    ...
    const { data: userDeposits } = useUserAcquisitionDeposits(testAddress)
...
export const useLockdropClaimsReady = () => {
    ...
    const testAddress = address || 'test_user_mock'
    const { data: userDeposits } = useUserAcquisitionDeposits(testAddress)
    const { deposits, totalPoints } = useAcquisition()
