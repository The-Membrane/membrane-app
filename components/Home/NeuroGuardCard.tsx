...
const NeuroGuardCloseButton = ({ guardedPosition, RBYield }: { guardedPosition: { position: PositionResponse, symbol: String, LTV: string, value: string }, RBYield: string }) => {
  ...
  return (<Card key={guardedPosition.position.position_id} width={"100%"} borderColor={""} borderWidth={3} padding={4}>
    <HStack gap={"11%"}>
      <Text variant="title" textAlign={"center"} fontSize={"lg"} letterSpacing={"1px"} width="55%" display="flex" justifyContent="center"> {guardedPosition.value} {guardedPosition.symbol} earning {num(RBYield).times(guardedPosition.LTV).toFixed(1)}%</Text>
      <TxButton
        maxW="25%"
