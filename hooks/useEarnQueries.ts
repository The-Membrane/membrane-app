...
export const useBoundedCDTVaultTokenUnderlying = (vtAmount: string) => {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            if (router.pathname != "/" && router.pathname != "/management") return
            if (!client) return
...
export const useBoundedCDTRealizedAPR = () => {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            var APR = num(currentClaimTracker.vt_claim_of_checkpoint).minus(rb_conversion_rates["119D"]).dividedBy(rb_conversion_rates["119D"])
            var negative = false
