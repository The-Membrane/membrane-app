...
const VENUE_CHOOSER_CARD_STYLE: React.CSSProperties = {
    height: 160,
    borderRadius: 14,
...
export const EmptyVenueChooser = ({
    ...
    return (
        <div style={{ gridColumn: '1 / -1' }}>
            <div style={VENUE_CHOOSER_CARD_STYLE}>
                <button type="button" aria-label="Prev option" onClick={() => setIdx(i => (i - 1 + options.length) % options.length)} style={{ padding: 8, borderRadius: 10, background: '#101731', border: '1px solid #2b2f47' }}>{'‹'}</button>
                <div style={{ textAlign: 'center' }}>
                    <div style={{ fontWeight: 700, marginBottom: 6 }}>Select Venue Type</div>
                    <div style={{ opacity: 0.85 }}>{current}</div>
                </div>
                <button type="button" aria-label="Next option" onClick={() => setIdx(i => (i + 1) % options.length)} style={{ padding: 8, borderRadius: 10, background: '#101731', border: '1px solid #2b2f47' }}>{'›'}</button>
            </div>
            ...
            <div style={{ marginTop: 10, display: 'flex', justifyContent: 'center' }}>
                <button type="button" onClick={() => onPlacementSelect(slot, current)} style={{ padding: '8px 12px', borderRadius: 10, background: '#1b2140', border: '1px solid #2b2f47' }}>Use This Placement</button>
            </div>
        </div>
    )
}
