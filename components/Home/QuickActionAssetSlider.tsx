...
export const QuickActionAssetWithSlider = ({ asset, label, onChangeExt }: AssetWithSliderProps) => {
  ...
  const onChange = (value: number) => {
    ...
    let updatedAssets = quickActionState.assets.map((asset) => {
      ...
      console.log(quickActionState.levAssets?.find((levAsset) => asset.symbol === levAsset.symbol), asset.symbol, quickActionState.levAssets)
      if(found > 0 && quickActionState.levAssets?.find((levAsset) => asset.symbol === levAsset.symbol)) quickActionState.levAssets[found] = asset
